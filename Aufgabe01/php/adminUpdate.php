...
if (isset($_POST['aktualisieren']))
{
    ...
    if (count($conditions) > 0)
    {
        ...
        $query .= "SET " . implode(',', $conditions) . " WHERE `ID` = '" . $_SESSION['userIDUpdate'] . "'";
    ...
    {
        $query = "SELECT * FROM `persons` WHERE `ID` = '" . $_SESSION['userIDUpdate'] . "' LIMIT 1";
        $result = mysqli_query(OpenCon() , $query);
        $count = mysqli_num_rows($result);
        if($_SESSION['userIDUpdate'] == $_SESSION['ID']){
...
if (isset($_POST['loeschen']))
{
    ...
    $query = " DELETE FROM `persons` WHERE `ID`= '" . $_SESSION['userIDUpdate'] . "'";
    $result = mysqli_query(OpenCon() , $query);
    if (!$result)
    {
        $updateMessage = 'FEHLER !!!';
    ...
    else
    {
        ...
        $updateMessage = 'Die Daten wurden erfolgreich gelöscht <br> Bitte cklicken Sie <a href = "home.php">hier</a> um die aktulalisierten Daten aufzurufen. ';
        if ($_SESSION['ID'] === $_SESSION['userIDUpdate'])
        {
